// assets/phoenix_live_view/js/phoenix_live_view/constants.js
...
// assets/phoenix_live_view/js/phoenix_live_view/utils.js
...
// assets/phoenix_live_view/js/phoenix_live_view/dom.js
var DOM = {
  // lvp:comment-next-line
  // byId(id) {
  // lvp:block-start
  // add domRoot param to search for the element inside the shadowDOM
  byId(id, domRoot) {
    const el = domRoot === void 0 ? document.getElementById(id) : domRoot.querySelector(`#${id}`);
    return el || logError(`no id found for ${id}`);
  },
  ...
  // lvp:comment-end
  // lvp:block-start
  isNowTriggerFormExternal(el, phxTriggerExternal, domBody) {
    return el.getAttribute && el.getAttribute(phxTriggerExternal) !== null && domBody.contains(el);
  },
  // lvp:block-end
...
// assets/phoenix_live_view/js/phoenix_live_view/upload_entry.js
...
// assets/phoenix_live_view/js/phoenix_live_view/live_uploader.js
...
// assets/phoenix_live_view/js/phoenix_live_view/aria.js
var ARIA = {
...
// assets/phoenix_live_view/js/phoenix_live_view/hooks.js
var Hooks = {
  LiveFileUpload: {
    activeRefs() {
...
// assets/phoenix_live_view/js/phoenix_live_view/element_ref.js
...
// assets/phoenix_live_view/js/phoenix_live_view/dom_post_morph_restorer.js
var DOMPostMorphRestorer = class {
  ...
  perform() {
    const container = dom_default.byId(this.containerId, this.domRoot);
    if (!container) {
      return;
    }
    ...
    if (this.updateType == "prepend") {
      this.elementIdsToAdd.reverse().forEach((elemId) => {
        maybe(
          this.domRoot.getElementById(elemId),
...
// assets/phoenix_live_view/js/phoenix_live_view/dom_patch.js
var DOMPatch = class {
    ...
    const morph = (targetContainer2, source, withChildren = this.withChildren) => {
      const morphCallbacks = {
        ...
        onNodeAdded: (el) => {
          ...
          if (dom_default.isNowTriggerFormExternal(el, phxTriggerExternal, view.domBody)) {
...
// assets/phoenix_live_view/js/phoenix_live_view/rendered.js
...
// assets/phoenix_live_view/js/phoenix_live_view/js.js
var focusStack = [];
...
var JS = {
  ...
  exec_push(e, eventType, phxEvent, view, sourceEl, el, args) {
    ...
    if (args.targetView && args.targetCtx) {
      ...
      view.withinTargets(phxTarget, handler, view.domRoot);
    }
  },
...
// assets/phoenix_live_view/js/phoenix_live_view/js_commands.ts
...
// assets/phoenix_live_view/js/phoenix_live_view/view_hook.ts
var HOOK_ID = "hookId";
var viewHookID = 1;
var ViewHook = class _ViewHook {
  ...
  handleEvent(event, callback) {
    ...
    this.__view().domRoot.addEventListener(
      `phx:${event}`,
  ...
  removeHandleEvent(ref) {
    this.__view().domRoot.removeEventListener(
      `phx:${ref.event}`,
      ref.callback
    );
...
// assets/phoenix_live_view/js/phoenix_live_view/view.js
...
var View = class _View {
  ...
  constructor(el, liveSocket, parentView, flash, liveReferer, domRoot) {
    this.domRoot = domRoot || document;
    this.domBody = domRoot || document.body;
    this.isDead = false;
    this.liveSocket = liveSocket;
    this.flash = flash;
  ...
  connectParams(liveReferer) {
    ...
    const manifest = dom_default.all(this.domRoot, `[${this.binding(PHX_TRACK_STATIC)}]`).map((node) => node.src || node.href).filter((url) => typeof url === "string");
  ...
  dropPendingRefs() {
    dom_default.all(this.domRoot, `[${PHX_REF_SRC}="${this.refSrc()}"]`, (el) => {
  ...
  attachTrueDocEl() {
    this.el = dom_default.byId(this.id, this.domRoot);
  ...
  // lvp:add-next-line - Use domRoot instead of document for Shadow DOM support
  execNewMounted(parent = this.domRoot) {
  ...
  onJoinError(resp) {
    ...
    if (this.isMain()) {
      ...
      const trueChildEl = dom_default.byId(this.el.id, this.domRoot);
      if (trueChildEl) {
  ...
  undoRefs(ref, phxEvent, onlyEls) {
    ...
    if (onlyEls) {
      ...
      dom_default.all(this.domRoot, selector, (parent) => {
      ...
      dom_default.all(this.domRoot, selector, (el) => this.undoElRef(el, ref, phxEvent));
    }
  }
  ...
  putRef(elements, phxEvent, eventType, opts = {}) {
    ...
    if (opts.loading) {
      const loadingEls = dom_default.all(this.domRoot, opts.loading).map((el) => {
  ...
  dispatchUploads(targetCtx, name, filesOrBlobs) {
    ...
    const inputs = dom_default.findUploadInputs(targetElement, this.domRoot).filter(
      (el) => el.name === name
    );
    if (inputs.length === 0) {
...
// assets/phoenix_live_view/js/phoenix_live_view/live_socket.js
...
var LiveSocket = class {
  constructor(url, phxSocket, opts = {}) {
    ...
    this.linkRef = 1;
    this.roots = {};
    this.href = opts.externalHref || window.location.href;
  ...
  joinDeadView() {
    const viewRoot = this.domRoot === document ? document.body : this.domRoot.querySelector(":scope > div");
    if (
      // lvp:comment-next-line
      // body &&
      // lvp:add-next-line
      viewRoot && // lvp:comment-next-line
      // !this.isPhxView(body) &&
      // lvp:add-next-line
      !this.isPhxView(viewRoot) && !this.isPhxView(document.firstElementChild)
    ) {
      const view = this.newRootView(viewRoot);
      view.setHref(this.getHref());
      view.joinDead();
      if (!this.main && !this.rootViewSelector) {
        this.main = view;
      }
      window.requestAnimationFrame(() => {
        view.execNewMounted();
  ...
  // lvp:block-start
  // from https://github.com/phoenixframework/phoenix_live_view/pull/3564
  viewSelector() {
  ...
  joinRootViews() {
    ...
    dom_default.all(
      this.domRoot,
      `${this.viewSelector()}:not([${PHX_PARENT_ID}])`,
      (rootEl) => {
  ...
  replaceMain(href, flash, callback = null, linkRef = this.setPendingLink(href)) {
    ...
    const stickies = dom_default.findPhxSticky(this.domRoot) || [];
  ...
  newRootView(el, flash, liveReferer) {
    const view = new View(el, this, null, flash, liveReferer, this.domRoot);
    ...
    for (const event in events) {
      ...
      this.on(browserEventName, (e) => {
        ...
        if (targetPhxEvent) {
          ...
          dom_default.all(this.domRoot, `[${windowBinding}]`, (el) => {
  ...
  // lvp:block-start
  bindPoint() {
    return this.domRoot === document ? window : this.domRoot;
  }
  // lvp:block-end
  bindClicks() {
  ...
  bindClick(eventName, bindingName) {
    ...
    this.bindPoint().addEventListener(
      eventName,
      (e) => {
        let target = null;
  ...
  dispatchEvent(event, payload = {}) {
    dom_default.dispatchEvent(this.domRoot, `phx:${event}`, { detail: payload });
  }
...
// assets/phoenix_live_view/js/phoenix_live_view/index.ts
