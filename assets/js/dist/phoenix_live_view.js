...
var DOMPostMorphRestorer = class {
  ...
  perform() {
    let container = dom_default.byId(this.containerId, this.domRoot);
...
var JS = {
  ...
  exec_exec(e, eventType, phxEvent, view, sourceEl, el, { attr, to }) {
    let nodes = to ? dom_default.all(view.domRoot, to) : [sourceEl];
    nodes.forEach((node) => {
  ...
  filterToEls(liveSocket, sourceEl, { to }) {
    let defaultQuery = () => {
      if (typeof to === "string") {
        return dom_default.all(liveSocket.domRoot, to);
      } else if (to.closest) {
...
// assets/phoenix_live_view/js/phoenix_live_view/view_hook.js
var HOOK_ID = "hookId";
var viewHookID = 1;
var ViewHook = class {
  static makeID() {
...
var View = class _View {
  ...
  connectParams(liveReferer) {
    ...
    let manifest = (
      ...
      // lvp:add-next-line
      dom_default.all(this.domRoot, `[${this.binding(PHX_TRACK_STATIC)}]`).map((node) => node.src || node.href).filter((url) => typeof url === "string")
    );
  ...
  onJoinError(resp) {
    ...
    if (this.isMain()) {
      ...
      let trueChildEl = dom_default.byId(this.el.id, this.domRoot);
      if (trueChildEl) {
  ...
  putRef(elements, phxEvent, eventType, opts = {}) {
    ...
    if (opts.loading) {
      let loadingEls = dom_default.all(this.domRoot, opts.loading).map((el) => {
...
var LiveSocket = class {
  ...
  joinDeadView() {
    let viewRoot = this.domRoot === document ? document.body : this.domRoot.querySelector(":scope > div");
  ...
  joinRootViews() {
    ...
    dom_default.all(this.domRoot, `${this.viewSelector()}:not([${PHX_PARENT_ID}])`, (rootEl) => {
    ...
    this.main.join((joinCount, onDone) => {
      if (joinCount === 1 && this.commitPendingLink(linkRef)) {
        this.requestDOMUpdate(() => {
          ...
          dom_default.findPhxSticky(this.domRoot).forEach((el) => newMainEl.appendChild(el));
  ...
  newRootView(el, flash, liveReferer) {
    let view = new View(el, this, null, flash, liveReferer, this.domRoot);
  ...
  bindTopLevelEvents({ dead } = {}) {
    ...
    this.on("drop", (e) => {
      ...
      let dropTarget = dropTargetId && dom_default.byId(dropTargetId, this.domRoot);
  ...
  bindClick(eventName, bindingName) {
    ...
    this.bindPoint().addEventListener(eventName, (e) => {
      let target = null;
  ...
  debounce(el, event, eventType, callback) {
    ...
    this.withinOwners(el, (view) => {
      let asyncFilter = () => !view.isDestroyed() && view.domBody.contains(el);
...
// assets/phoenix_live_view/js/phoenix_live_view/index.js
