...
export default class View {
  ...
  connectParams(liveReferer) {
    ...
    // lvp:add-next-line
    const manifest = DOM.all(this.domRoot, `[${this.binding(PHX_TRACK_STATIC)}]`)
  ...
  onJoinError(resp) {
    ...
    if (this.isMain()) {
      ...
      // lvp:add-next-line
      const trueChildEl = DOM.byId(this.el.id, this.domRoot);
      if (trueChildEl) {
  ...
  putRef(elements, phxEvent, eventType, opts = {}) {
    ...
    if (opts.loading) {
      ...
      // lvp:add-next-line
      const loadingEls = DOM.all(this.domRoot, opts.loading).map((el) => {
  ...
  dispatchUploads(targetCtx, name, filesOrBlobs) {
    ...
    // lvp:add-next-line
    const inputs = DOM.findUploadInputs(targetElement, this.domRoot).filter(
      (el) => el.name === name,
    );
    if (inputs.length === 0) {
